...
import { enrollmentProceduresContent } from "@/data/content";

export default function EnrollmentProceduresPage() {
  const { navigation, hero, process, beforeYouBegin, cta } =
    enrollmentProceduresContent;
        ...
        <div className="grid gap-16 lg:grid-cols-[220px_1fr] lg:gap-24">
          ...
          <aside className="lg:sticky lg:top-32 lg:h-fit">
            ...
            <nav className="flex gap-1 overflow-x-auto pb-2 lg:block lg:overflow-visible">
              {navigation.map((item) => {
                const active = item.id === "enrollment";
          ...
          <div className="min-w-0 max-w-4xl">
            ...
            <AdmissionsSection
              eyebrow={beforeYouBegin.eyebrow}
              title={beforeYouBegin.title}
              description={beforeYouBegin.description}
            >
              <div className="grid border-t border-black/10 sm:grid-cols-2">
                {beforeYouBegin.items.map((item) => (
                  <div
                    key={item.number}
                    className="border-b border-black/10 py-8 sm:px-6 sm:first:pl-0 sm:even:border-l"
                  >
                    <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-black/35">
                      {item.number}
                    </p>
