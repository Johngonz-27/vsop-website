        ...
        <div className="grid gap-16 lg:grid-cols-[220px_1fr] lg:gap-24">
          ...
          <aside className="lg:sticky lg:top-32 lg:h-fit">
            ...
            <nav className="flex gap-1 overflow-x-auto pb-2 lg:block lg:overflow-visible">
              {navigation.map((item) => {
                const active = item.id === "requirements";
          ...
          <div className="min-w-0 max-w-4xl">
            {/* EDUCATIONAL LEVELS */}
            <AdmissionsSection
              eyebrow={educationalLevels.eyebrow}
              title={educationalLevels.title}
              description={educationalLevels.description}
            >
              <div className="border-t border-black/10">
                {educationalLevels.items.map((item, index) => (
                  <div
                    key={item.level}
            ...
            {/* GENERAL DOCUMENTS */}
            <AdmissionsSection
              eyebrow={generalDocuments.eyebrow}
              title={generalDocuments.title}
              description={generalDocuments.description}
            >
              <div className="border-t border-black/10">
                {generalDocuments.items.map((item, index) => (
                  ...
                  >
                    <span className="text-[11px] font-semibold tracking-[0.12em] text-black/35">
                      {String(index + 1).padStart(2, "0")}
                    </span>
