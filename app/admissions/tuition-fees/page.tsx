...
import { tuitionFeesContent } from "@/data/content";
      ...
      <div className="mx-auto w-full max-w-7xl px-8 py-20 sm:px-10 md:px-12 md:py-28">
        <div className="grid gap-16 lg:grid-cols-[220px_1fr] lg:gap-24">
          ...
          <div className="min-w-0 max-w-4xl">
            {/* TUITION INFORMATION */}
            <AdmissionsSection
              eyebrow={tuitionInformation.eyebrow}
              title={tuitionInformation.title}
              description={tuitionInformation.description}
            >
              <div className="border-t border-black/10">
                {tuitionInformation.levels.map((item, index) => (
                  <div
                    key={item.level}
            ...
            {/* JUNIOR HIGH */}
            <AdmissionsSection
              eyebrow={juniorHigh.eyebrow}
              title={juniorHigh.title}
              description={juniorHigh.description}
            >
            ...
            >
              <div className="grid border-t border-black/10 sm:grid-cols-2">
                {seniorHigh.strands.map((strand, index) => (
                  <Link
                    key={strand.href}
                    href={strand.href}
                    className={`group border-b border-black/10 py-8 ${
                      index === 0 ? "sm:border-r sm:pr-8" : "sm:pl-8"
                    }`}
                  >
                    <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-black/35">
                      {strand.label}
