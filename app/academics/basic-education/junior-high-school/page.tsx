...
import { juniorHighSchoolContent } from "../../../../data/content";
...
export default function JuniorHighSchool() {
  const content = juniorHighSchoolContent;
    ...
    <Layout>
      ...
      <section className="bg-[#f7f8f3]">
        <div className="mx-auto max-w-7xl px-6 py-20 sm:px-8 md:py-24 lg:px-10">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.14em] text-[#035921]">
              {content.academicProgram.number} —{" "}
              {content.academicProgram.eyebrow}
            </p>

            <h2 className="mt-4 text-3xl font-semibold tracking-tight text-[#0d0d0d] sm:text-4xl">
              {content.academicProgram.title}
            </h2>

            <p className="mt-5 text-base leading-7 text-[#626862]">
              {content.academicProgram.description}
            </p>
          </div>

          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {content.academicProgram.items.map((item) => {
              const Icon =
        ...
        <div className="mx-auto max-w-7xl px-6 py-20 sm:px-8 md:py-24 lg:px-10">
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            <div className="lg:pr-8">
              <p className="text-sm font-semibold uppercase tracking-[0.14em] text-[#035921]">
                {content.classroomCommunity.number} —{" "}
                {content.classroomCommunity.eyebrow}
              </p>

              <h2 className="mt-4 text-3xl font-semibold tracking-tight text-[#0d0d0d] sm:text-4xl">
                {content.classroomCommunity.title}
              </h2>

              {content.classroomCommunity.paragraphs.map((paragraph) => (
                <p
                  key={paragraph}
                  className="mt-5 text-base leading-8 text-[#626862]"
                >
                  {paragraph}
                </p>
              ))}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {content.classroomCommunity.items.map((item, index) => (
                <div
                  key={item}
      ...
      {/* =========================================================
          06 — PREPARING FOR SENIOR HIGH SCHOOL
      ========================================================= */}
          ...
          <div className="grid gap-12 lg:grid-cols-[0.75fr_1.25fr]">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.14em] text-[#b8d7b7]">
                {content.seniorHighPreparation.number} —{" "}
                {content.seniorHighPreparation.eyebrow}
              </p>

              <h2 className="mt-4 max-w-md text-3xl font-semibold tracking-tight text-white sm:text-4xl">
                {content.seniorHighPreparation.title}
              </h2>

              <p className="mt-5 max-w-md text-base leading-8 text-white/70">
                {content.seniorHighPreparation.description}
              </p>
            </div>

            <div className="grid gap-8 sm:grid-cols-2">
              {content.seniorHighPreparation.items.map((item) => (
                <div
                  key={item.number}
